...
import asyncio
import math
from utils import is_visible_by_path, get_window_from_path
...
# Traps
SELECT_TRAPS = frozenset(['Hex', 'Disarming Trap', 'Curse', 'Spirit Trap', 'Death Trap', 'Life Trap', 'Myth Trap', 'Storm Trap', 'Fire Trap', 'Backdraft', 'Feint', 'Ice Trap'])
...
class SlackFighter(CombatHandler):
	...
	@logger.catch()
	async def wait_for_next_round(self, current_round: int, sleep_time: float = 1):
		"""
	...
	@logger.catch()
	async def wait_for_combat(self, sleep_time: float = 1):
		"""
	...
	async def discard_spell_in_hanging_effect(self, spell):
		...
		for i in self.hanging_effect_IDs:
			if i[0] == card_atr[0] and i[1] == card_atr[1]:
				...
				await spell.discard(sleep_time = 1)
				break
		...
		while discarded:
			...
			for c in self.cards.copy():
				if all([self.card_names[c] not in s for s in master_list]):
					...
					await c.discard(sleep_time=0.5)
			...
			for t in strategy:
				...
				if spell_matches:
					...
					for selected_spell in spell_pip_value_in_order:
						# print(self.card_names[selected_spell])
						...
						if not selected_ally :
							...
							allies_to_compare = [a for a in self.allies if await a.is_player() == True if await self.member_participants[a].primary_magic_school_id() == selected_spell_school or selected_spell_school == 1027491821 or self.card_names[selected_spell] in non_balance_uni_blade_names]
...
#big thanks to major for doing most of the work, & click for helping
	...
	async def will_kill(self, target: CombatMember, card: CombatCard, enchant: CombatCard = None) -> bool:
	...
	async def damage_calc_handle_round(self, cards):
		...
		if damagest_card:
			if result := await self.will_kill(self.selected_enemy, damagest_card, enchant_damage):
